...
class BisindoBoundingBoxOverlay extends StatelessWidget {
  final BisindoParseResult result;
  final int modelInputSize;

  const BisindoBoundingBoxOverlay({
    Key? key,
  ...
  Widget build(BuildContext context) {
    return Positioned.fill(
      child: CustomPaint(
        painter: BisindoBoundingBoxPainter(
          result: result,
...
class BisindoBoundingBoxPainter extends CustomPainter {
  final BisindoParseResult result;
  final Size imageSize;

  BisindoBoundingBoxPainter({required this.result, required this.imageSize});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final boxColor = result.label.isWord ? const Color(0xFF0984E3) : const Color(0xFF00B894);
  ...
  @override
  bool shouldRepaint(covariant BisindoBoundingBoxPainter oldDelegate) {
    return true;
  }
}
