...
/// Layanan (Service) yang diisolasi murni untuk mengatur instansiasi Model TFLite YOLOv8.
...
class TFLiteService {
  ...
  Future<void> loadModel(String modelPath) async {
    try {
      ...
      debugPrint("DEBUG [TFLiteService]: Model Loaded! Input Shape: $inputShape, Type: ${inputTensor.type}, isNchw: $_isNchw, size: $_modelInputSize");
    } catch (e) {
  ...
  Future<ParseResult?> runInference(
      List<List<List<List<double>>>> inputTensor, List<SignLabel> labels) async {
    ...
    try {
      ...
      debugPrint("DEBUG [TFLiteService]: Memulai run() TFLite...");
      _interpreter!.run(inputTensor, output);
      debugPrint("DEBUG [TFLiteService]: Selesai run(). Output shape awal (array): $shape");

      // 3. Decoding List hasil TFLite kembali di Background Isolate 
      //    untuk mencegah frame UI patah-patah (Jank).
      final parseData = ParseData(output as List<List<List<double>>>, shape, labels);
      final result = await compute(parseYoloOutput, parseData);
