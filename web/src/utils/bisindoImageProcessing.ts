 ...
 * @param targetSize Ukuran input model (default BISINDO_MODEL_INPUT_SIZE = 640)
 * @returns Object Tensor float32 [1, 3, targetSize, targetSize]
 */
export function extractBisindoNchwTensor(
  video: HTMLVideoElement,
  ctx: CanvasRenderingContext2D,
  targetSize: number = BISINDO_MODEL_INPUT_SIZE,
  isFrontCamera: boolean = true
): Tensor {
  // 1. Gambar video ke canvas (Mirror Horizontal Opsional)
