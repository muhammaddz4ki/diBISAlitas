import {
  BisindoSignLabel,
  BISINDO_CONFIDENCE_THRESHOLD,
} from "../constants/bisindoLabels";

export interface BisindoDetectionResult {
  box: [number, number, number, number]; // [xCenter, yCenter, width, height] dalam skala 640x640
  score: number;
  label: BisindoSignLabel;
}
...
): BisindoDetectionResult[] {
  // Urutkan dari skor tertinggi ke terendah
  const sorted = [...detections].sort((a, b) => b.score - a.score);
  const kept: BisindoDetectionResult[] = [];
 ...
 * [1, 52, 8400] atau [1, 8400, 52]) dan mengekstrak semua Bounding Box
 ...
 * - Jika dim[1] < dim[2] → format [1, 52, 8400] (transposed, atribut di dim 1)
 * - Jika dim[1] > dim[2] → format [1, 8400, 52] (normal, box di dim 1)
 *
 * @param data Array Float32 murni dari output tensor YOLOv8
 * @param outDims Dimensi output tensor, misalnya [1, 52, 8400]
 ...
 */
export function parseBisindoDynamicTensor(
  data: Float32Array,
  outDims: readonly number[],
  labels: BisindoSignLabel[],
  confidenceThreshold: number = BISINDO_CONFIDENCE_THRESHOLD
): BisindoDetectionResult[] {
  // Jika dim[1] lebih kecil dari dim[2], misal [1, 52, 8400] -> transposed
  ...
  const candidates: BisindoDetectionResult[] = [];
