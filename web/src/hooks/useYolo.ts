...
import { extractNchwTensor } from "../utils/imageProcessing";
import { parseDynamicTensor, DetectionResult } from "../utils/yoloInference";
import { CONFIDENCE_THRESHOLD, MODEL_INPUT_SIZE, SignLabel } from "../constants/signLabels";
 ...
 * @param labels Kamus array SignLabel yang digunakan model ini
 * @returns State dari deteksi (loading, error, hasil deteksi array + best, dan fungsi pemicu deteksi per frame)
 */
export function useYolo(modelPath: string | null, labels: SignLabel[]) {
  ...
  useEffect(() => {
    ...
    const loadModel = async () => {
      ...
      try {
        ...
        if (isMounted) {
          setError("Gagal memuat model YOLOv8 (Inference Session Error).");
          setIsModelLoading(false);
        }
      }
    };
  ...
  const detectFrame = useCallback(
    async (video: HTMLVideoElement, ctx: CanvasRenderingContext2D, isFrontCamera: boolean = true) => {
      ...
      try {
        ...
        const inputTensor = extractNchwTensor(video, ctx, MODEL_INPUT_SIZE, isFrontCamera);
        ...
        const allDetections = parseDynamicTensor(data, outDims, labels, CONFIDENCE_THRESHOLD);
