...
import { BisindoDetectionResult } from "../utils/bisindoYoloInference";
import { useBisindoCamera } from "../hooks/useBisindoCamera";
import { BISINDO_MODEL_INPUT_SIZE } from "../constants/bisindoLabels";

interface BisindoCameraDetectorProps {
  isModelLoading: boolean;
  error: string | null;
  detectFrame: (video: HTMLVideoElement, ctx: CanvasRenderingContext2D, isFrontCamera: boolean) => Promise<void>;
  detectionResult: BisindoDetectionResult | null;
  /** Array deteksi penuh setelah NMS (opsional, untuk render multi-box) */
  detections?: BisindoDetectionResult[];
}

/**
 * Komponen presentasional yang menyatukan input video kamera dengan kanvas overlay bounding box.
 */
export function BisindoCameraDetector({
  isModelLoading,
  error,
  detectFrame,
  detectionResult,
  detections,
}: BisindoCameraDetectorProps) {
  const videoRef = useRef<HTMLVideoElement>(null);
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const requestRef = useRef<number>(0);

  // Menyuntikkan custom hook useCamera untuk siklus hidup aliran media
  const { isCameraReady, cameraError, toggleCamera, facingMode } = useBisindoCamera(videoRef, canvasRef);
  ...
  }, [isModelLoading, isCameraReady, detectFrame, cameraError, error]);
  ...
  useEffect(() => {
    ...
    const results: BisindoDetectionResult[] = detections && detections.length > 0
      ? detections
      : detectionResult
        ? [detectionResult]
        : [];
    ...
    const scaleX = canvasW / BISINDO_MODEL_INPUT_SIZE;
    const scaleY = canvasH / BISINDO_MODEL_INPUT_SIZE;
    ...
    for (const detection of results) {
      ...
      // Desain Bounding Box Premium (Beda Warna untuk Huruf vs Kata Kerja)
      const boxColor = label.isWord ? "#0984E3" : "#00B894";
      ...
      const labelText = `${label.label} ${(score * 100).toFixed(1)}%`;
    ...
    <div className="flex-1 relative bg-black w-full h-full">
      ...
      {(!isCameraReady || isModelLoading) && !error && !cameraError && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/80 z-30">
          <div className="w-10 h-10 border-4 border-[#0984E3] border-t-transparent rounded-full animate-spin"></div>
          <p className="text-white font-medium mt-4 tracking-wide text-sm">
            {isModelLoading ? "Memuat AI YOLOv8 BISINDO..." : "Memulai Kamera..."}
          </p>
        </div>
      )}
